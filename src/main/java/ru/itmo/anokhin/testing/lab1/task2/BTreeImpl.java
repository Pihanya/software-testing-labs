...
public class BTreeImpl<K extends Comparable<K>, V> implements BTree<K, V> {
  ...
  BTreeNode<K, V> rootNode; // root of the B-tree
  int treeHeight; // height of the B-tree
  int entriesAmount; // number of key-value pairs in the B-tree
  ...
  BTreeImpl(BTreeNode<K,V> rootNode) {
  ...
  private BTreeNode<K, V> split(final BTreeNode<K, V> splitNode) {
    ...
    splitTo.setKeysAmount(splitNode.getKeysAmount() - halfIndex);
